...
public class ProjectPane extends JPanel implements Observer, ChangeListener {
	...
	public ProjectPane(ProjectManager projectManager, AbstractFileManager fileManager, Properties properties,
			Database database) {
		...
		tabbedPane = new SingleRowTabbedPane(SingleRowTabbedPane.FOUR_BUTTONS, SwingConstants.RIGHT);
	...
	private void createPanes() {
		...
		while (iter.hasNext()) {
			...
			tabbedPane.addTab(project.getShortName(), (Icon) icons.get(new Integer(project.getType())),
	...
	private JPanel createPanel(Project project) {
		...
		case Project.TYPE_DESIGN:
			return new DesignPane((Design) project, database, fileManager, properties);
		}
