    ...
    public void paint(Graphics g) {
        ...
        if (type > -1) {
            if (type == Node.TYPE_RESERVOIR) {
                Line2D l = new Line2D.Double(getWidth() / 2, getHeight() / 2,
                        getWidth(), getHeight() / 2);
                g2.draw(l);
            }

            if (type == Node.TYPE_DISPATCH) {
                Line2D l = new Line2D.Double(0, getHeight() / 2, getWidth(),
                        getHeight() / 2);
                g2.draw(l);
            }

            if (type == Node.TYPE_FAUCET) {
                Line2D l = new Line2D.Double(0, getHeight() / 2,
            ...
            g2.drawString(getText(), x - ((w - 10) / 2),
                ((y - ((h - 4) / 2)) + fm.getAscent()) - 3);
        }
