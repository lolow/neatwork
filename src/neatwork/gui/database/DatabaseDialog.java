...
public class DatabaseDialog extends JDialog implements Observer {
	...
	public DatabaseDialog(FrameNeatwork frame, Database database0) {
		...
		actions[0].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DatabaseDialog.Insert_a_new_diameter"));

		// DeleteDiam
		actions[1] = jTableDiameter.deleteRowAction;
		actions[1].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DatabaseDialog.Delete_selected_diameter(s)"));
		...
		actions[6].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DatabaseDialog.Load_data"));
		actions[6].putValue(Action.SMALL_ICON, new ImageIcon(getClass().getResource(pathImage + "DataExtract.png")));

		// check
		Icon icon = new ImageIcon(getClass().getResource(pathImage + "DataStore.png"));
		actions[7] = new NeatworkAction(Messages.getString("DatabaseDialog.Apply"), icon,
		...
		// save tableau
		Icon icon3 = new ImageIcon(getClass().getResource(pathImage + "report.png"));
		actions[9] = new NeatworkAction(Messages.getString("DatabaseDialog.Export"), icon3,
			...
			public void actionPerformed(ActionEvent e) {
				...
				s += Messages.getString("DatabaseDialog.Database_-_Diameters");
				s += "</title></head><body>";
				s += ("<h1>" + Messages.getString("DatabaseDialog.Database_-_Diameters") + "</h1>"); // $NON-NLS-3$
				...
				for (Enumeration en = database.getDiameters(); en.hasMoreElements();) {
		...
		jTabbedPane.addTab(Messages.getString("DatabaseDialog.Diameters"), new FancyTablePanel(
				Messages.getString("DatabaseDialog.Diameters_list"), actions, jTableDiameter, true));
		...
		actions[0].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DatabaseDialog.Insert_a_new_orifice"));

		// DeleteOrifice
		actions[1] = jTableOrifice.deleteRowAction;
		actions[1].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DatabaseDialog.Delete_selected_orifice(s)"));
		...
		//
		icon = new ImageIcon(getClass().getResource(pathImage + "DataStore.png"));
		actions[7] = new NeatworkAction(Messages.getString("DatabaseDialog.Apply"), icon,
				Messages.getString("DatabaseDialog.Save_data"), 'A') {
			public void actionPerformed(ActionEvent e) {
		...
		actions[9] = new NeatworkAction(Messages.getString("DatabaseDialog.Export"), icon3,
			...
			public void actionPerformed(ActionEvent e) {
				...
				s += Messages.getString("DatabaseDialog.Database_-_Orifices");
				s += "</title></head><body>";
				s += ("<h1>" + Messages.getString("DatabaseDialog.Database_-_Orifices") + "</h1>"); // $NON-NLS-3$
				s += "<table BORDER CELLPADDING=0 CELLSPACING=0>";
				s += Messages.getString("DatabaseDialog.<tr><th>Diameter</th></tr>");

				for (Iterator en = database.getOrifices().iterator(); en.hasNext();) {
		...
		jTabbedPane.addTab(Messages.getString("DatabaseDialog.Orifices"),
				Tools.getPanelTable(Messages.getString("DatabaseDialog.<html>Orifices_list"), actions, jTableOrifice));
