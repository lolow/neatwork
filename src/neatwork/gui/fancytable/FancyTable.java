...
public class FancyTable extends JTable {
	...
	public FancyTable(FancyTableModel fancyTableModel, boolean updateColor) {
		...
		Icon icon = new ImageIcon(getClass().getResource(pathImage + "NewRow.png"));
		insertRowAction = new AbstractAction("+", icon) {
			public void actionPerformed(ActionEvent e) {
				addNewRow();
			}
		};
		icon = new ImageIcon(getClass().getResource(pathImage + "DeleteRow.png"));
		...
		copyAction.putValue(Action.SHORT_DESCRIPTION, Messages.getString("FancyTable.Copy"));
				...
				try {
					...
					if (!FancyTable.this.fancyTableModel.setPaste(paste)) {
						JOptionPane.showMessageDialog(null,
								Messages.getString("FancyTable.Clipboard_is_not_well_formated"),
								Messages.getString("FancyTable.Format_error"), JOptionPane.ERROR_MESSAGE);
					} else {
		...
		pasteAction.putValue(Action.SHORT_DESCRIPTION, Messages.getString("FancyTable.Paste"));
		autoFit();
	...
	public void addNewRow() {
		...
		SwingUtilities.invokeLater(new Runnable() {
			public void run() {
				scrollRectToVisible(getCellRect(fancyTableModel.getRow(), 1, true));
			}
		});
