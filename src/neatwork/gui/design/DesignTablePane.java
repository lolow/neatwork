...
public class DesignTablePane extends JPanel implements Observer {
	...
	public DesignTablePane(Design design, Database database) {
		...
		actions[0].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DesignTablePane.Insert_a_new_node"));

		// DeleteNode
		actions[1] = jTableNode.deleteRowAction;
		actions[1].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DesignTablePane.Delete_selected_node(s)"));
		...
		actions[6].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DesignTablePane.Undo_modifications"));

		// check
		Icon icon = new ImageIcon(getClass().getResource("/neatwork/gui/images/Check.gif"));
		actions[7] = new NeatworkAction(Messages.getString("DesignTablePane.Apply"), icon,
				Messages.getString("DesignTablePane.Apply_modifications"), 'A') {
			public void actionPerformed(ActionEvent e) {
				apply();
			}
		};
		jSplitPane.setTopComponent(
				new FancyTablePanel(Messages.getString("DesignTablePane.Node_list"), actions, jTableNode));
		...
		actions[0].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DesignTablePane.Insert_a_new_pipe"));

		// DeletePipe
		actions[1] = jTablePipe.deleteRowAction;
		actions[1].putValue(Action.SHORT_DESCRIPTION, Messages.getString("DesignTablePane.Delete_selected_pipe(s)"));
		...
		jSplitPane.setBottomComponent(
				new FancyTablePanel(Messages.getString("DesignTablePane.Arc_list"), actions, jTablePipe));
	...
	private void updateCostLabel() {
		costLabel.setText(Messages.getString("DesignTablePane.Design_total_cost")
	...
	private void apply() {
		...
		String content = design.getPropertiesContent() + nodesTableModel.getContent() + pipesTableModel.getContent();
