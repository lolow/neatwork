...
public class CoreTopographie {
	...
	/**
	 * construit une topographie a l'aide d'un vecteur.
	 * <p>
	 * Le format du vecteur est: node id, pred node, height, length, # taps.<br>
	...
	public CoreTopographie(Vector data) {
		...
		for (Enumeration e = data0.elements(); e.hasMoreElements();) {
			...
			if (tap < 0) {
				throw new RuntimeException("fichier topo non valide (number of taps <0)");
			}
