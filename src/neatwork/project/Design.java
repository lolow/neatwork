...
public class Design extends Network {
	...
	private String getNodeCSV(Node node) {
		return node.getName() + "," + node.getHeight() + "," + node.getCoordX() + "," + node.getCoordY() + ","
				+ node.getOrifice() + "," + node.getComercialOrifice() + "," + node.getType();
	}
	...
	private String getPipeCSV(Pipe pipe) {
		return pipe.getBegin() + "," + pipe.getEnd() + "," + pipe.getLength() + "," + pipe.getLength1() + ","
				+ pipe.getRefDiam1() + "," + //$NON-NLS-1$
	...
	private String getDiameterCSV(String ref) {
		...
		return ref + "," + diam.getNominal() + "," + diam.getSdr() + "," + //$NON-NLS-3$
				diam.getDiameter() + "," + diam.getCost() + "," + diam.getMaxLength() + "," + diam.getType() + ","
				+ diam.getRoughness() + "," + "N"; //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public String getPropertiesContent() {
		...
		while (iter.hasMoreElements()) {
			...
			if (name.startsWith("topo.") && name.endsWith(".value")) {
				content += (name.substring(5, name.length() - 6) + "," + getProperties().getProperty(name) + "\n");
			}
		}
	...
	public Hashtable getSummary() {
		...
		for (Iterator i = getPipeIterator(); i.hasNext();) {
			...
			if (!p.getRefDiam1().equals("0")) {
				h.put(p.getRefDiam1(), new Double(((Double) h.get(p.getRefDiam1())).doubleValue() + p.getLength1()));
			}

			if (!p.getRefDiam2().equals("0")) {
				h.put(p.getRefDiam2(), new Double(((Double) h.get(p.getRefDiam2())).doubleValue() + p.getLength2()));
			}
		}
	...
	public String extractTopoContent() {
		...
		while (iter.hasNext()) {
			...
			if ((item.getBegin().indexOf("_") < 0) && (item.getEnd().indexOf("_") < 0)) {
